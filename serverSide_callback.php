	...
	if (count($linea) > 0){
		...
		for ($i = 0; $i < count($vecTemp); $i++){			
			...
			if ( is_numeric(substr($vecTemp[$i],0,$e)) && substr($vecTemp[$i],0,$e) > 24) {
				...
				$vecAgentes[$contA][2] = "<span style='font-size:9px; color:green'>00:00:00</span>";
				...
				if (substr($vecTemp[$i],69,2) == "OK" || substr($vecTemp[$i],88,2) == "OK" || substr($vecTemp[$i],95,2) == "OK" || substr($vecTemp[$i],6,2) == "ON"){
					...
					for ($j = 0; $j < count($vecTemp1); $j++){
						if (beginsWith( trim($vecTemp1[$j]), "SIP/".$vecAgentes[$contA][0]."-" ) || beginsWith( trim($vecTemp1[$j]), "IAX2/".$vecAgentes[$contA][0]."-" ) || beginsWith( trim($vecTemp1[$j]), "Zap/".($vecAgentes[$contA][0]-300)."-" )){
							...
							if ($vecCanal[4] == "Ring" || $vecCanal[4] == "Ringing"){
								...
								$vecAgentes[$contA][2] = "<span style='font-size:9px; color:black'>".date("H:i:s", ($base + $vecCanal[11] ))."</span>";
	   ...
	   <tr>
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[$i][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+1)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+2)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+3)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+4)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+5)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		<td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+6)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
        ...
        <td width="117" align="center" valign="middle" style="font-family:verdana; font-size:12px">
			<div style="background:url(images/<?php echo $vecAgentes[($i+7)][1]; ?>);width:111px; height:45px; border:1px solid white">
				<div style="text-align:center; margin-top:9px; margin-left:10px">
		...
		if (count($vecZAP) > 0){
			echo '<tr><td colspan=8 align=center><strong>L&iacute;neas Disponibles</strong></td></tr>';					
		?>
		<tr><td colspan="8" align="center">
			<table border="0">
...
elseif ($_GET['tipo'] == "queueControl"){
	...
	if (count($linea) > 0){
		...
		$contBusy = 0;
		$contTT = 0;
		$vecQ = str_replace("Agent", "", $linea['queue']);
		...
		for ($i = 0; $i < count($vecTemp); $i++){			
			if (strpos($vecTemp[$i],"/") > -1){
				...
				else if (strpos($vecTemp[$i], "agentes/n") > -1)
				{
					if (strpos($vecTemp[$i],"Busy)") > 0 || strpos($vecTemp[$i],"(In use)") ) {
						$toPublish .= '
						<td width="171" align="center" valign="middle" style="font-family:verdana; font-size:12px">
						<div style="background:url(images/queue_busy.jpg);width:171px; height:41px; border:1px solid white">
							<div style="text-align:center; margin-top:13px; margin-left:10px">
		...
		<tr>
		<td align='left' valign='top' style='font-family:verdana; font-size:13px; border:1px solid #CCCCCC' width='322'>
		...
		<div style='background-color:#ECEBF3; border:1px solid #CCC; margin-bottom: 5px'>
			...
			<img src='images/stats.png' align='absmiddle'> <strong>Estad&iacute;sticas Generales</strong>
			</div>
		</div>
		<table border='0' width='100%'>
			...
			<tr>
				...
				<td  style='font-family:verdana; font-size:12px;'>".substr($vecStat[4],3)."</td>
			</tr>						
		</table>
		<br>
		...
		";			
		for ($i = 0; $i < count($vecTemp); $i++){
			if (strpos($vecTemp[$i],"gent/") > 0)
				break;
				
			if (strpos($vecTemp[$i],"(wait:")){
				echo "<img src='images/incoming.png' align='absmiddle'> Llamada ".substr($vecTemp[$i], 0, strpos($vecTemp[$i],".")+1)." Tiempo en Espera: ".substr($vecTemp[$i],strpos($vecTemp[$i],"wait")+6, (strpos($vecTemp[$i],", prio")-strpos($vecTemp[$i],"wait")-6))."<br>";
				$contA++;
			}			
		}
		echo "<br><strong>Llamadas Totales en Espera: ".$contA."</strong><br><br>";

		echo "
		</td>
		<td valign='top' rowspan='2' align='right'>
		<table width='99%' border=\"0\" cellpadding='1' cellspacing='2' style=\"border:1px solid #CCCCCC\">
			<tr>".$toPublish."
			</tr>
		</table>
		</td>
...
elseif ($_GET['tipo'] == "guardarExt"){
	...
	if ($_GET["prot"] == "SIP"){
		...
		while ($linea = mysql_fetch_array($result, MYSQL_ASSOC)){
			...
			if ($linea['usuario'] > 500)
			{
				$sipdata .= "disallow = all\nallow = g729\nallow = alaw\n";
			}
			$sipdata .= "\n";
...
elseif ($_GET['tipo'] == "peer"){
	...
	if ($_GET['idPeer'] >= 300 && $_GET['idPeer'] < 400){
		$cad  = $_GET['idPeer']."|";
		$cad .= $linea['secret']."|";
		$cad .= "ZAP|";
...
elseif ($_GET['tipo'] == "loadTroncal"){
	...
	$resultado = $linea['nombre']."+".$linea['host']."+".$linea['type']."+".$linea['codec']."+".$linea['protocolo']."+".$linea['prefijo']."+".$linea['usuario']."+".$linea['password']."+".$linea['dtmf']."+".$linea['contexto'];
	echo $resultado;
}
...
elseif ($_GET['tipo'] == "updateTroncal"){
	...
	if (trim($_GET['nombre']) != ""){
		$sSQL = "INSERT INTO troncal (nombre,host,type,codec,protocolo,prefijo,usuario,password, dtmf, contexto) VALUES ('".$_GET['nombre']."', '".$_GET['host']."',	'".$_GET['type']."','".$_GET['codec']."','".$_GET['protocolo']."','".$_GET['prefijo']."','".$_GET['usuario']."','".$_GET['password']."','".$_GET['dtmf']."', '".$_GET['contexto']."') ON DUPLICATE KEY UPDATE
		host='".$_GET['host']."',type='".$_GET['type']."',codec='".$_GET['codec']."',protocolo='".$_GET['protocolo']."',prefijo='".$_GET['prefijo']."',usuario='".$_GET['usuario']."',password='".$_GET['password']."', dtmf='".$_GET['dtmf']."', contexto='".$_GET['contexto']."'";
		...
		if ($_GET['protocolo'] != "ZAP"){
			...
			while($linea = mysql_fetch_array($result, MYSQL_ASSOC)){
				...
				if (trim($linea['usuario']) != "" && trim($linea['password']) != ""){
...
exten => _[1-5]XX,n,Macro(marcacionInterna,\${EXTEN},R)

exten => _[1-5]XXX,1,Playback(advertencia)
exten => _[1-5]XXX,n,Macro(marcacionInterna,\${EXTEN},R)

exten => _[1-5]XXXX,1,Playback(advertencia)
exten => _[1-5]XXXX,n,Macro(marcacionInterna,\${EXTEN},R)

exten => _6XX,1,Playback(advertencia)
exten => _6XX,n,Macro(marcacionInterna,MATH(${EXTEN}-600,i),R)
...
; Direccionamiento dinámico
".$faxLine."
exten => _[1-5]XX,1,Macro(marcacionInterna,\${EXTEN},N)
exten => _[1-5]XXX,1,Macro(marcacionInterna,\${EXTEN},N)
exten => _[1-5]XXXX,1,Macro(marcacionInterna,\${EXTEN},N)
exten => _6XX,1,Macro(marcacionInterna,MATH(${EXTEN}-600,i),N)
